...
**To upload the data to GCS, I used this code.** [here](./web_to_gcs.py)

**To create the tables in BigQuery, I used these queries.** [here](./Queries.sql)
...
1. Create a new model `fct_taxi_trips_quarterly_revenue.sql` [here](./taxi_rides_ny/models/core/fct_taxi_trips_quarterly_revenue.sql)
...
WITH RankedYoY AS (
    ...
    FROM `dbt_vquijada.fct_taxi_trips_quarterly_revenue`
    WHERE year = 2020
)
SELECT service_type, year_quarter, yoy_growth
FROM RankedYoY
...
1. Create a new model `fct_taxi_trips_monthly_fare_p95.sql` [here](./taxi_rides_ny/models/core/fct_taxi_trips_monthly_fare_p95.sql)
...
SELECT 
    ...
    MAX(p95) AS max_p95, 
    MAX(p90) AS max_p90
FROM `dbt_vquijada.fct_taxi_trips_monthly_fare_p95`
...
Prerequisites:
* Create a staging model for FHV Data (2019), and **DO NOT** add a deduplication step, just filter out the entries where `where dispatching_base_num is not null` [here](./taxi_rides_ny/models/staging/stg_fhv_tripdata.sql)
* Create a core model for FHV Data (`dim_fhv_trips.sql`) [here](./taxi_rides_ny/models/core/dim_fhv_trips.sql) joining with `dim_zones`. Similar to what has been done [here](../../../04-analytics-engineering/taxi_rides_ny/models/core/fact_trips.sql)
* Add some new dimensions `year` (e.g.: 2019) and `month` (e.g.: 1, 2, ..., 12), based on `pickup_datetime`, to the core model to facilitate filtering for your queries

Now...
1. Create a new model `fct_fhv_monthly_zone_traveltime_p90.sql` [here](./taxi_rides_ny/models/core/fct_fhv_monthly_zone_traveltime_p90.sql)
...
FROM (
    ...
    FROM `dbt_vquijada.fct_fhv_monthly_zone_traveltime_p90`
